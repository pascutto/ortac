...
module Full_report = struct
  ...
  let value ppf (value : Translated.value) =
    pf ppf
      ...
      \  @[%a@]@\n\
       - Return:@\n\
      \  @[%a@]@\n"
      pp_loc value.loc value.name value.pure terms value.preconditions terms
      value.postconditions xposts value.xpostconditions arguments
      value.arguments return_pattern value.returns
  ...
  let constant ppf (constant : Translated.constant) =
    pf ppf
      ...
      \  @[%a@]@\n\
       - Invariants:@\n\
      \  @[%a@]" pp_loc constant.loc constant.name terms constant.checks
      invariants constant.type_.invariants
  ...
  let type_ ppf (type_ : type_) =
    pf ppf
      ...
      \  @[%a@]@\n\
       ...
       - Comparison: %a@\n\
       - Copy: %a@\n"
      pp_loc type_.loc type_.name invariants type_.invariants derived
      type_.equality derived type_.comparison derived type_.copy
